...
class OrderPatchSchema(Schema):
    ...
    payment_type_id = fields.Int(allow_none=True)
    delivery_fee_final = fields.Decimal(as_string=True, places=2, allow_none=True)
    delivery_fee_override_reason = fields.Str(allow_none=True)
...
class OrderSchema(LocalDateTimeMixin, Schema):
    ...
    global_discount_amount = fields.Decimal(as_string=True, places=2)
    global_discount_reason = fields.Str(allow_none=True)
    subtotal_before_payment_surcharge = fields.Decimal(as_string=True, places=2, dump_only=True)
    payment_type_name_snapshot = fields.Str(dump_only=True)
    payment_surcharge_type_snapshot = fields.Str(dump_only=True)
    payment_surcharge_value_snapshot = fields.Decimal(as_string=True, places=4, dump_only=True)
    payment_surcharge_amount = fields.Decimal(as_string=True, places=2, dump_only=True)
